...
const VoteForm = (props) => {
    ...
    const [trainer] = useState(props.location.state.trainer)
    const [course] = useState(props.location.state.course)
    const [courseDate] = useState(props.location.state.end)
    const [courseId] = useState(props.location.state.id)
    ...
    const onSubmit = async () => {
        ...
        console.log(vote)
        await axios
            .post("http://localhost:5000/votes/add/", vote)
            .then((result) => console.log("x" + result.data));

        window.location = "/c_list"
    }
